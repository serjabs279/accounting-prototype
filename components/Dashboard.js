...
export default defineComponent({
  ...
  setup() {
    ...
    return { summary, state, openManualEntry, chartData, todayReport };
  },
  render() {
    const stats = [
      { label: 'Total Assets', value: this.summary.totalAssets, color: 'text-emerald-700', symColor: 'text-emerald-200', sub: 'School Properties' },
      { label: 'Annual Revenue', value: this.summary.totalRevenue, color: 'text-emerald-700', symColor: 'text-emerald-200', sub: 'Collection' },
      { label: 'Total Expenses', value: this.summary.totalExpenses, color: 'text-rose-600', symColor: 'text-rose-200', sub: 'Operations' },
      { label: 'Total Debit', value: this.summary.totalSystemDebit, color: 'text-slate-900', symColor: 'text-emerald-300', sub: 'System Volume' },
      { label: 'Total Credit', value: this.summary.totalSystemCredit, color: 'text-slate-900', symColor: 'text-emerald-300', sub: 'System Volume' },
      { label: 'Fiscal Surplus', value: this.summary.netIncome, color: 'text-slate-900', symColor: 'text-slate-200', sub: 'Institutional Health' }
    ];
    ...
    return h('div', { class: 'animate-fade space-y-10' }, [
      // Top Stats Row - Updated to grid-cols-6
      h('div', { class: 'grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4' }, stats.map(s => 
        h('div', { class: 'premium-card p-6 transition hover:shadow-2xl flex flex-col justify-center' }, [
